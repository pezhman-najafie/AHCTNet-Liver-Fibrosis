...
\alpha\_{\text{image}} + \alpha\_{\text{frequency}} = 1
...
α\_image + α\_frequency = 1

The spatial component is computed as:

Spatial = α\_image × Input
...
These frequency regions are multiplied by α\_frequency and concatenated with the spatial image.
...
models.convnext\_tiny(

    weights=models.ConvNeXt\_Tiny\_Weights.DEFAULT
...
    "cuda" if torch.cuda.is\_available() else "cpu"
...
|---|---:|---:|---:|
| Accuracy | 0.9921 | 0.0021 | **99.21% ± 0.21%** |
| Weighted Precision | 0.9922 | 0.0021 | **99.22% ± 0.21%** |
| Weighted Recall | 0.9921 | 0.0021 | **99.21% ± 0.21%** |
| Weighted F1 Score | 0.9921 | 0.0021 | **99.21% ± 0.21%** |
| Balanced Accuracy | 0.9881 | 0.0033 | **98.81% ± 0.33%** |
| Matthews Correlation Coefficient (MCC) | 0.9897 | — | **98.97%** |
| Cohen's Kappa | — | — | Not reported in the supplied final output |
| Multiclass ROC-AUC (OvR) | — | — | Not reported in the supplied final output |

> **Note:** These values correspond to validation performance aggregated across the five stratified cross-validation folds. They should not be interpreted as performance on an independent external test dataset.

Performance Summary

- **Accuracy:** 99.21% ± 0.21%
- **Precision:** 99.22% ± 0.21%
- **Recall:** 99.21% ± 0.21%
- **F1 Score:** 99.21% ± 0.21%
...
The spatial-frequency fusion coefficients are optimized jointly with the rest of the network through the learnable Softmax-normalized fusion parameters.
...
Because the two fusion weights satisfy:

$$
\alpha_{\text{image}} + \alpha_{\text{frequency}} = 1
$$

the reported mean frequency contribution of approximately **48.95%** corresponds to an inferred spatial contribution of approximately **51.05%**.

The near-balanced learned weighting suggests that the trained model makes substantial use of both the original spatial representation and the FFT-derived frequency information rather than strongly suppressing either component.

Final numerical results are reported using the mean and standard deviation across all five validation folds.
